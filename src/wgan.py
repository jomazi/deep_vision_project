...
CREDENTIALS: @martinarjovsky --> https://github.com/martinarjovsky/WassersteinGAN
"""
...
import fcts.dcgan as dcgan
import fcts.mlp as mlp
...
if __name__=="__main__":
    ...
    parser.add_argument('--dataset', required=True, help='cifar10 | lsun | imagenet | folder | lfw | LLD')
    ...
    elif opt.dataset == 'LLD':
        trafo_train = transforms.Compose([transforms.ToTensor(), transforms.Normalize((.5,), (.5,))])
        dataset = IconDataset(transform=trafo_train)
        dataset = torch.utils.data.Subset(dataset, np.arange(50000, dtype=int).tolist())
    ...
    gen_iterations = 0
    for epoch in tqdm(range(opt.niter), desc='Train', leave=False):
